...
类型对照表是 [`docs/types.yaml`](types.yaml)。评测机只读这份文件。

允许的签名类型：叶子 `int` / `long` / `float` / `bool` / `str`，以及一层、两层 `List[T]`（含 `List[float]`、`List[bool]`）。不要三层列表、不要链表/树节点。

当前这些类型对八种语言都能接上，所以题目下拉默认可以有 Python 3、C、C++20（内部 id 仍是 `cpp17`）、JavaScript、TypeScript、Go、Rust、Zig，再去掉这台机器没装编译器的语言。某类型若某语言接不上，该语言**不会出现在本题下拉**，不是灰色的 `NA`。

值按签名类型与参数顺序写入 `meta.yaml`：

| 键 | 适用 |
| --- | --- |
| `scale_min` / `scale_max` | 输入规模：图有几个点，或数组/字符串有多长。没有数组、字符串、图时不写 |
| `bounds` | 每个参数自己的数或字符：整数/小数写该参数的 `min`/`max`，字符串写 `charset`。边列表前「有几个点」写在 `scale_min` / `scale_max` |

`meta.languages` 可用来限制本题能交的语言（如只允许 `python3`）。不写 = 签名接得上的全部语言。网页下拉 = 本题列表里、评测机已经实现、并且本机已装好编译器的语言。提交一种不在列表里的语言会 400「本题不支持该语言」。
...
- `scale_min` / `scale_max`：输入规模的下界/上界。图题写点数；有数组或字符串时写长度。纯整数/小数/`bool` 不写。
- `bounds`：每个参数自己的数或字符，键是 `signature.yaml` 的 `name`。整数/小数写 `min`/`max`，字符串写 `charset`。
- `languages`：可选。本题允许的语言 id 列表。缺省不要写。

## statement.md

Markdown 题面。`# 标题` 下直接写说明（参数和返回值写进这段），再接 `## 示例` 和 `## 约束`。用户不读写标准输入，只实现函数。
...
支持的 `type` 见上一节与 [`docs/types.yaml`](types.yaml)。默认用 `int`；仅答案或参数会超 32 位有符号整数时用 `long`。C 的二维展开为指针加行数、列数。链表、二叉树尚未支持。三层列表不支持。
...
公开测例 2～3 条，与题面示例一致。提交用隐藏测例。

## starter

空函数模板：只给出类/函数签名，函数体留空，不要写 `return {}`、`return false` 等占位返回值。Python 空缩进块是语法错误，提交空 starter 得到 `CE` 即可。

评测机管理页「按文件上传」成功后可点「生成 starter」，实现见 [`backend/app/services/starters.py`](../backend/app/services/starters.py)。只读签名，不读 `tests.jsonl`。不要手抄。
...
```bash
cd ~/leet-judge         # 实际克隆路径；WSL 用 Linux 家目录，不要 /mnt/c
